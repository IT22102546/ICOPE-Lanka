...
const OTPRequest = () => {
  ...
  const sendOtp = async () => {
    ...
    // Convert isEmail from string to boolean
    const isEmailBool = isEmail === "true";

    try {
      const response = await fetch(
        `${API}/api/v1/auth/send-password-reset-otp`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            isEmail: isEmailBool, // Use boolean value
          }),
        }
      );
  ...
  try {
    const response = await fetch(
      `${API}/api/v1/auth/send-password-reset-otp`,
      {
        ...
        body: JSON.stringify({
          identifier: emailOrPhone, // ✅ Change from emailOrPhone to identifier
          isEmail: isEmailBool,
        }),
      }
    );
 ...
 const verifyOtp = async (enteredOtp: string) => {
  ...
  try {
    const response = await fetch(
      `${API}/api/v1/auth/verify-password-reset-otp`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
